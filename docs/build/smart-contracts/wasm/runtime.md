...
Archethic leverages execution of smart contracts through WebAssembly.

Contracts are then defined as WASM module and could be coded multiple languages.

The validators hold a WebAssembly runtime able to execute the code for the smart contract.

The runtime is responsible of:
- instanciation of the WASM modules
- managing the communication between the host (blockchain) and the WASM module
- sharing a piece of memory for input/output of the contract
...
To facilitate the communication and implementation then interaction between validation and wasm module is done through JSON.
...
Then any smart contract must define a specification or ABI (Application Binary Interface) defined in the manifest of the smart contract.
...
Several kind of functions are then reserved for this purpose: `onInit`, `onUpgrade`, `onInherit`

- **onInit**

This function is called during the transaction’s validation and for the first contract of the chain. This behaves as a constructor or initializer of the contract’s state, which will be injected into the transaction’s validation as unspent output.
...
flowchart LR
  ...
  Runtime -- onInit --> WASM
  WASM -- State --> Runtime
  Runtime -- UTXO --> Transaction
```

- **onUpgrade**

As Archethic’s upgrade is native, a specific and magic trigger is supported as `code_upgrade` to correspond to the AEIP-22 - Smart Contract upgradability spec and will replace the new code of the code.

This hook function is called during the execution of this `code_upgrade` function to generate the new state to be injected into the transaction’s validation as new unspent output.

```mermaid
flowchart LR
  Transaction --> Runtime
  Runtime -- onUpdate --> WASM
  WASM -- State --> Runtime
  Runtime -- UTXO --> Transaction
```
...
This function ensures a way to assert some conditions along the chain’s inheritance (i.e. origin family devices, hard-coded rules, etc.).
